...
- audit and understand the real-world image dataset through exploratory data analysis
- benchmark multiple deep learning approaches under the same preprocessing pipeline
- select a final model using a defensible evaluation rule
- strengthen the project with robustness evidence, Grad-CAM explainability, and a basic deployment path
...
EDA was performed before training to understand the practical difficulties of the dataset. The analysis confirmed that all images share a native resolution of 1500 x 1500 pixels, which simplifies resizing and preprocessing. However, the dataset also showed three important challenges:

- strong class imbalance
- frequent image blur
- variable field backgrounds with clutter and non-leaf content

These findings were important because they directly informed the model and evaluation strategy. In particular, they justified the use of class balancing, augmentation, macro-F1 as a key metric, and a later robustness analysis that went beyond clean validation accuracy.
...
To reduce the effect of class imbalance, minority classes were augmented until each class reached 748 training samples, matching the largest original class. This produced a balanced training set of 5,236 images.

Training-time augmentation included resizing, random horizontal flipping, random rotation, and mild color jitter. Validation samples were kept closer to their original form so that measured performance remained representative of held-out evaluation rather than heavy augmentation.
...
- a Hybrid CNN-Transformer using EfficientNetB0 features plus a lightweight transformer encoder

The benchmark used accuracy, macro-F1, and inference latency. Macro-F1 was emphasized because the dataset contains minority classes and a model that performs well only on majority classes would not be practically reliable.

The locked final evaluation rule used the validation holdout from original images. Models were ranked by:
...
The final selected Hybrid CNN-Transformer was evaluated under five conditions:
...
## 7. Explainability and Deployment
...
Deployment packaging was also prepared directly from the notebook. The final package includes:

- `class_info.json` with class names and disease notes
- `sample_prediction.json` with a top-3 prediction example
- a single-image prediction figure
- standalone `predict.py` and `app.py` scripts in the project root

The notebook attempted ONNX export, but this step was skipped in the recorded execution because `onnxscript` was not installed in that environment.
...
This Markdown version is submission-ready from a technical content perspective. Before final PDF or DOCX export, the remaining academic finishing step is to insert the final APA 7 references from the literature review source files and align the formatting with the required institutional template.
