...
Previous research in plant disease recognition has shown that CNNs and transfer learning perform strongly when images are captured in controlled or semi-controlled settings. However, many studies underrepresent the practical difficulties of field deployment, especially blur, clutter, inconsistent lighting, and minority-class scarcity. This project uses that gap as its motivation and frames the present study as a practical benchmark on uncontrolled potato leaf imagery rather than a clean-dataset-only experiment.
...
### 3.3 Preprocessing and Balancing
