...
Draft status:
- Built from the current audited workspace state on April 3, 2026
...
To address this problem, four deep learning models were benchmarked: a simple CNN trained from scratch, EfficientNetB0 with a frozen backbone, EfficientNetB0 with fine-tuning, and a Hybrid CNN-Transformer model. A balanced training strategy was applied by augmenting minority classes to 748 samples per class, resulting in a balanced set of 5,236 training images. Benchmark evidence from the main notebook showed that the Hybrid CNN-Transformer achieved the strongest overall performance, with an accuracy of 0.9292 and macro-F1 of 0.9373, while the fine-tuned EfficientNetB0 also performed strongly with an accuracy of 0.9209 and macro-F1 of 0.9294. These results indicate that combining convolutional feature extraction with transformer-based contextual modeling is effective for disease identification in uncontrolled agricultural imagery.
...
## 5. Results
Benchmarking evidence from the notebook showed clear separation between the weaker baseline and the stronger transfer-learning and hybrid models. The simple CNN achieved an accuracy of 0.6507, macro-F1 of 0.6544, and latency of 162.86 ms per image. This confirmed that training from scratch on this dataset was substantially weaker than using pretrained features.

EfficientNetB0 with a frozen backbone improved performance to an accuracy of 0.7496 and macro-F1 of 0.7382 while also reducing latency to 69.83 ms per image. This result showed the benefit of transfer learning even when the backbone weights are largely fixed. Fine-tuning EfficientNetB0 provided a large additional gain, reaching an accuracy of 0.9209 and macro-F1 of 0.9294 at 78.00 ms per image.

The strongest benchmarked model was the Hybrid CNN-Transformer, which achieved an accuracy of 0.9292 and macro-F1 of 0.9373 with latency of 109.32 ms per image. Although its latency was higher than the fine-tuned EfficientNetB0, the hybrid model produced the best overall classification quality. Under the current project evidence, this makes it the strongest candidate for the final selected model.
...
The benchmarking results show that pretrained feature extraction is essential for this task. The large gap between the baseline CNN and both EfficientNet-based models indicates that the dataset is too challenging for a simple model trained from scratch to perform competitively. The improvement from frozen EfficientNetB0 to fine-tuned EfficientNetB0 further shows that partial task-specific adaptation is valuable. Finally, the Hybrid CNN-Transformer achieved the strongest macro-F1, which suggests that global contextual modeling contributes additional discriminative power beyond the CNN-only pipeline.
...
The strongest observed result came from the Hybrid CNN-Transformer, which achieved 0.9292 accuracy and 0.9373 macro-F1, outperforming the CNN and transfer-learning baselines. These findings suggest that combining a CNN backbone with transformer-based contextual reasoning is a strong approach for challenging agricultural imagery. The remaining project work is centered on final robustness evidence, explainability outputs, deployment packaging, and final report integration. Even so, the current benchmark evidence already demonstrates that reliable disease identification on uncontrolled field images is achievable with an appropriately designed deep learning model.

## 9. Limitations and Future Work
This draft should acknowledge several limitations clearly. First, the strongest numerical evidence currently comes from notebook outputs, and some final packaging steps still need to be completed locally. Second, the current report draft is based on validation-stage evidence rather than a fully finalized held-out test narrative. Third, while the notebook now includes sections for robustness, Grad-CAM, and deployment packaging, the corresponding output files still need to be generated and integrated into the final report.
