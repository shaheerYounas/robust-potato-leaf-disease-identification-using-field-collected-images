...
The specific objectives of the project are to audit and understand the field-collected dataset through exploratory data analysis, benchmark multiple deep learning approaches under the same preprocessing pipeline, identify a best-performing model using defensible evaluation criteria, and prepare robustness, explainability, and deployment components that strengthen the project beyond raw accuracy alone.

## 2. Literature Review

Prior studies in plant disease recognition consistently show that CNNs, transfer learning, and careful preprocessing can produce strong predictive results. At the same time, the literature also shows that many systems are validated on relatively clean datasets with less visual noise than real agricultural imagery. That gap matters here because the current dataset was captured under uncontrolled field conditions, making robustness and balanced evaluation more important than raw accuracy alone.

The present project is therefore positioned as a practical field-imagery benchmark and deployment-oriented study. It uses the literature to motivate stronger model comparison, explainability, and deployment readiness rather than a narrow benchmark-only narrative.
...
### 5.1 Robustness, Explainability, and Deployment
...
At the time of this draft, the wording in this section should be treated as provisional until the newest notebook cells are executed and their outputs are saved. The intended final report content should include a robustness table comparing clean validation images against blur, low-light, noise, and occlusion conditions, Grad-CAM examples demonstrating whether the selected model focuses on lesion regions rather than background clutter, and deployment artifacts including class metadata, example single-image prediction output, and ONNX export status.
...
- Replace provisional wording in the robustness, explainability, and deployment discussion after the newest notebook sections are executed.
- Insert formal APA 7 references from the literature review files.
- Align the final exported version with the institutional submission template.
